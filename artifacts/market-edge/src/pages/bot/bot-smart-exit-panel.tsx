              ...
              <tbody>
                {(status?.evaluations ?? []).map((evaluation) => (
                  <tr key={evaluation.orderId} className="border-t border-amber-200/[0.06] text-amber-50/75">
                    ...
                    <td className="px-3 py-2 font-mono">{evaluation.projectedCrossingSeconds?.toFixed(1) ?? "—"}s <span className="text-amber-100/30">/ {evaluation.secondsRemaining.toFixed(1)}s</span><div className="text-[8px] text-amber-100/30">{evaluation.projectionState ?? "—"} · reserve {evaluation.reserveSeconds ?? "—"}s</div><div className="text-[8px] text-amber-100/30">breach {evaluation.targetBreachConfirmationCount ?? 0} · {evaluation.targetBreachSpanMs == null ? "—" : `${(evaluation.targetBreachSpanMs / 1_000).toFixed(1)}s`} · value {evaluation.quoteLagProtectionEligible ? `protected ≥${((evaluation.quoteLagProtectionFloor ?? 0) * 100).toFixed(1)}¢` : "not protected"}</div></td>
