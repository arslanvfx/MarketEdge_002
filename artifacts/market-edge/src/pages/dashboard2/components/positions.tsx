...
import { Activity, ChevronDown, ChevronUp, Loader2, AlertTriangle } from "lucide-react";

export default function PositionsTab({ mode }: { mode: 'paper' | 'live' }) {
  ...
  if (isLoading && !positions) {
    return (
      <div className="flex h-[300px] items-center justify-center text-slate-500 gap-3 bg-[#0f141d] border border-slate-800/60 rounded-lg">
        ...
        <span className="font-mono text-sm tracking-widest uppercase">Loading Positions...</span>
      </div>
    );
  }
  ...
  if (isError && !positions) {
    return (
      <div className="flex h-[300px] items-center justify-center text-rose-500 gap-3 bg-[#0f141d] border border-slate-800/60 rounded-lg">
        ...
        <span className="font-mono text-sm tracking-widest uppercase">Failed to load positions</span>
      </div>
    );
  }
  ...
  return (
    <div className="bg-[#0f141d] border border-slate-800/60 rounded-lg flex flex-col overflow-hidden h-full">
      <div className="p-4 border-b border-slate-800/60 bg-black/20 flex justify-between items-center">
        ...
        <span className="text-[10px] font-mono text-amber-500 bg-amber-500/10 px-2.5 py-1 rounded border border-amber-500/20">
        ...
        <table className="w-full text-left border-collapse">
          <thead className="bg-[#0a0e14] sticky top-0 z-10 border-b border-slate-800/60 shadow-sm">
            <tr className="text-[10px] uppercase tracking-wider text-slate-500">
              ...
              <th className="px-4 py-3 font-semibold text-right">Fills / Req</th>
                ...
                <React.Fragment key={p.id}>
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right">
                      {p.side === 'yes' && <span className="text-[10px] font-bold text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">YES</span>}
                      {p.side === 'no' && <span className="text-[10px] font-bold text-rose-400 bg-rose-500/10 px-2 py-0.5 rounded border border-rose-500/20">NO</span>}
                    </td>
                    <td className="px-4 py-3 text-right">
                      <span className="font-mono text-sm text-slate-200">{p.filled_contracts}</span>
                      <span className="font-mono text-xs text-slate-600"> / {p.requested_contracts}</span>
                    </td>
                    <td className="px-4 py-3 text-right">
                    ...
                    <tr className="bg-black/20 border-b border-slate-800/40">
                      <td colSpan={7} className="px-4 py-4">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 pl-8">
                          <div>
                            <span className="block text-[10px] uppercase tracking-wider text-slate-500 mb-1">Order ID</span>
                            <span className="font-mono text-xs text-slate-400">{p.order_id || "—"}</span>
                          </div>
                          <div>
                            <span className="block text-[10px] uppercase tracking-wider text-slate-500 mb-1">Client Order ID</span>
                            <span className="font-mono text-xs text-slate-400">{p.client_order_id || "—"}</span>
                          </div>
                          <div>
