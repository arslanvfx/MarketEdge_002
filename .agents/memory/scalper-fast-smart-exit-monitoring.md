...
Smart Exit must also bound all of its database work below the shared pool capacity, with ownership/request lifecycle work prioritized ahead of observational writes. Normal Scalper entry claims stay outside that queue so exit telemetry cannot consume every connection or starve entry cap reservations.

**Why:** Production showed many confirmed Scalper fills but zero dedicated exit evaluations. Spot requests used display symbols instead of products, and the monitor iterated every historical unsettled row sequentially before reaching current exposure. During hardening, non-aborted deadline failures could pin coalesced requests across the whole short position lifetime, aggregate-only overload hid which positions missed their SLA, Pyth's whole-second timestamp alone could not distinguish genuine updates from repeated payloads, and an unbounded telemetry flush could compete with entry claims on the small shared database pool.

**How to apply:** Keep product identity, exact ticker/window identity, active-window filtering, mode filtering, concurrent active evaluation, abortable generation-safe coalescing, per-order overload state, provider-native source continuity, and bounded priority-aware database use at the monitor boundary. Historical reconciliation, settlement, and deferred telemetry writes belong on a separate maintenance lane and must not occupy the hot path or exhaust shared database capacity.