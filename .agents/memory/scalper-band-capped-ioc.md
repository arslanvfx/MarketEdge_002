...
Qualify a Scalper candidate against the final authenticated quote, but submit the IOC at the pinned price-band ceiling as the worst acceptable winning-contract cost. YES uses that ceiling directly; NO uses its exact symmetric YES-price complement. Size contracts and check balance against the same worst-case cost, not the more favorable observed quote. Funding requirements include the upward-rounded worst-case taker fee; the final live cash check also keeps a one-cent safety margin.

**Why:** Submitting at the exact transient quote caused repeated zero fills when the market moved one cent before Kalshi handled the IOC. The band ceiling makes those in-band moves marketable without relaxing the configured policy or budget. Principal-only sizing can pass a near-balance order while omitting its fee, causing a definitive insufficient-balance rejection. An unknown result can also arise before any network call, so it cannot prove that a locally persisted limit was submitted.
