...
- [Auto-pilot architecture](autopilot-architecture.md) — training coins (BTC/ETH/XRP/HYPE/BNB) always run Claude; auto-pilot evaluates per-coin edge and routes the Auto-Pilot consensus signal; non-training coins are stat-only by policy (no API cost); autopilot.ts is the pure decision engine
