...
The hot ticker lane is isolated from slower full-evidence collection. Coinbase REST ticker `time` is a last-trade timestamp and may freeze for tens of seconds on thinner products, so the hot lane uses successful receipt time as observation time while exact duplicate prices remain non-distinct trajectory samples. Per-position evaluation is serialized/coalesced. Active regular and Scalper positions must be serviced before inactive prewarming and share the 500 ms hot lane. Slow spot, tape, and L2 inputs are reusable only when transport and exchange event times are fresh. Missing volatility, momentum, or book imbalance makes a crossing UNAVAILABLE; trade flow is optional only in the critical band when all other independent and execution evidence agrees.
...
**Why:** The user confirmed that 15-minute markets need room to recover early. In one production window, three of four positions signaled: two eventual winners exited on tiny crosses while held-side Kalshi probability was flat near 96–99%, and ETH exited on a 0.0072% cross with 231 seconds left. A later production window showed fresh HTTP responses paired with 42–53-second-old Coinbase trade timestamps, erasing hot-lane trajectories during the final minutes.
