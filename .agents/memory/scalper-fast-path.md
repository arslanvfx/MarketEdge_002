...
**How to apply:** Keep the exact final identity refresh, authenticated orderbook, fresh Freefall/target-distance sample, final balance, advisory-locked daily/open cap claim, immutable risk revalidation, intent-before-live-POST, IOC cost ceiling, and strict unknown reconciliation. Claim aggregates may share one locked SQL round trip. Dashboard timing starts at cached-candidate detection, so it does not include time a scan request waited in the coalesced pending slot. Measure the final authenticated requote separately from the earlier parallel refresh, and retain the market-close budget at detection/completion so late results identify whether the attempt itself crossed expiry.