...
## 8. Phase 2 — Late-window re-entry (recovery scalp)

Once the exit engine above has taken you out of a bad trade, there's a second, independent opportunity: once the market's direction is effectively decided, the *opposite* side can be bought cheaply and will converge to $1 at settlement with very low remaining variance. This is not "revenge trading" if it's gated correctly — it's a distinct, low-variance trade with its own edge calculation. If gated wrong, it's a martingale trap. Build it as follows.

### 8.1 Entry gate (reuses the same `P_win` engine from Section 3)

Do **not** gate this on a wall-clock rule like "wait until the last minute." Time-remaining alone doesn't tell you whether direction is actually decided — a volatile asset can flip in the final 30 seconds while a calm one can be functionally decided with 3 minutes left. Gate on the model, not the clock:

```
P_win_new_side = NormalCDF(distance_new / sigma_window)   # same formula as Section 3, flipped side

ENTER recovery scalp only if ALL of:
  P_win_new_side          >= high_confidence_threshold     # e.g. 0.95–0.97
  sigma_window            <= max_residual_vol               # confirms low reversal risk, not just "time is short"
  continuation_score      confirms direction still holding (Section 3 formula, same-signed)
  market_price_new_side   <= P_win_new_side - min_edge - est_fees_and_slippage
```

The last line matters most in practice: if the market has already priced the new side at 98¢, there's nothing left to capture after fees — skip the trade even if everything else qualifies.

### 8.2 Sizing rule — the trap to avoid

This trade must be sized as its own independent bet, using your normal sizing rules for a high-confidence/low-payout trade (small edge, small consistent size). It must **never** be sized as a function of how much was lost on the original trade. If any part of the sizing logic references cumulative loss-to-date or "amount to recover," remove it — that's a martingale pattern and it's the single fastest way to turn a sound tactic into an account-blowing one.

### 8.3 Recommended engine flow

```
on_original_position_exit(state):
    watch_for_recovery_entry = True

def on_tick_recovery(state):
    if not watch_for_recovery_entry:
        return
    P_new = norm_cdf(distance_new_side / sigma_window)
    if (P_new >= high_confidence_threshold
        and sigma_window <= max_residual_vol
        and continuation_confirmed(continuation_score)
        and market_price_new_side <= P_new - min_edge - fees):
            size = fixed_recovery_size    # independent of prior loss, from normal sizing table
            return ENTER(new_side, size)
    return WAIT
```

### 8.4 Parameters to add to the tuning list (Section 5)

- `high_confidence_threshold` — minimum model win probability to qualify (start ~0.95)
- `max_residual_vol` — ceiling on `sigma_window` to confirm reversal risk is genuinely low
- `min_edge` — minimum gap between model probability and market price after fees
- `fixed_recovery_size` — pulled from your normal sizing table, never derived from loss-to-date

Backtest this phase the same way as Phase 1: score on whether it adds positive expected value on its own, independent of the original trade's outcome. If it doesn't stand on its own as a +EV trade, it shouldn't be in the system.

---

## 9. One caveat worth stating plainly
